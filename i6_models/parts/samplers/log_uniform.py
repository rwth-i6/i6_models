__all__ = ["LogUniformSampler", "LogUniformSamplerWithoutReplacement"]
...
class LogUniformSampler(nn.Module):
    def __init__(
        ...
        *,
        ...
        ws = torch.arange(
            self.num_classes, dtype=torch.get_default_dtype(), device=device
        )
        self._distribution = (torch.log1p(ws + 1) - torch.log1p(ws)) / torch.log1p(
        ...
        self._cat_sampler = torch.distributions.categorical.Categorical(
...
class LogUniformSamplerWithoutReplacement(nn.Module):
    def __init__(
        self,
        num_classes: int,
        *,
        ...
        # self._cat_sampler = AliasMultinomial(probs=self._distribution)
