...
def test_ConformerConvolutionV2():
    def get_output_shape(batch, time, features, norm=None, kernel_size=31, dropout=0.1, activation=nn.functional.silu, dropout_broadcast_axes=None):
        x = torch.randn(batch, time, features)
        if norm is None:
            norm = nn.BatchNorm1d(features)
        cfg = ConformerConvolutionV2Config(
            channels=features, kernel_size=kernel_size, dropout=dropout, activation=activation, norm=norm, dropout_broadcast_axes=dropout_broadcast_axes
        )
...
def test_ConformerPositionwiseFeedForwardV2():
    def get_output_shape(input_shape, input_dim, hidden_dim, dropout, activation, dropout_broadcast_axes=None):
        ...
        cfg = ConformerPositionwiseFeedForwardV2Config(
            input_dim=input_dim, hidden_dim=hidden_dim, dropout=dropout, activation=activation, dropout_broadcast_axes=dropout_broadcast_axes
        )
...
def test_ConformerMHSARelPosV1():
    def get_output_shape(input_shape, seq_len, input_dim, num_att_heads=8, 
                         att_weights_dropout=0.1, dropout=0.1, 
                         learnable_pos_emb=True, rel_pos_clip=16, with_pos_bias=False, pos_emb_dropout=0.0, dropout_broadcast_axes=None):
        assert len(input_shape) == 3 and input_shape[-1] == input_dim

        cfg = ConformerMHSARelPosV1Config(input_dim=input_dim, num_att_heads=num_att_heads, 
                                                            att_weights_dropout=att_weights_dropout, dropout=dropout,
                                                            learnable_pos_emb=learnable_pos_emb, rel_pos_clip=rel_pos_clip, 
                                                            with_pos_bias=with_pos_bias, pos_emb_dropout=pos_emb_dropout,
    ...
    for learnable_pos_emb, with_pos_bias, pos_emb_dropout in product([True, False], [True, False], [0.0, 0.1]):
        assert get_output_shape(input_shape, seq_len, 32, learnable_pos_emb=learnable_pos_emb, 
                                with_pos_bias=with_pos_bias, pos_emb_dropout=pos_emb_dropout) == [4, 15, 32]
